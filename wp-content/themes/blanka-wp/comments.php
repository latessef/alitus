...
<?php if (have_comments()) : ?>
        <div class="block center-relative content-740">
    ...
    if (comments_open()) :
        ...
        echo '<div class="block center-relative content-740">';
        ...
        comment_form(
                array('fields' => array(
                        'author' => '<input id="author" name="author" type="text" placeholder="' . esc_html__('Name', 'blanka-wp') . '" value="' . esc_attr($commenter['comment_author']) . '" size="20"' . $aria_req . ' />',
                        'email' => '<input id="email" name="email" type="text" placeholder="' . esc_html__('Email', 'blanka-wp') . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="20"' . $aria_req . ' />'
                    ),
                    'comment_field' => '<textarea id="comment" placeholder="' . esc_html__('Write Comment...', 'blanka-wp') . '" name="comment" cols="45" rows="12" aria-required="true"></textarea>',
                    'title_reply' => '',
                    'comment_notes_before' => '',
                    'comment_notes_after' => '',
                    'label_submit' => esc_html__('Send', 'blanka-wp')));
        echo '</div></div>';
    endif;
    ...
    <?php if ($comment->comment_approved == '0') : ?>
                    <em><?php echo esc_html__('Your comment is awaiting moderation.', 'blanka-wp'); ?></em>
                    <br /> <br />
                <?php endif; ?>
